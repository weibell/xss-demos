(() => {
  ...
  // Parse "a=1&b=2" style string into object
  const parseKV = s => (s||'')
    .replace(/^[?#]/,'')
    .split('&')
    .filter(Boolean)
  ...
  // Params from current URL (? and #)
  const urlParams = { ...parseKV(location.search), ...parseKV(location.hash) };

  // Globals (if defined)
  const globalParams = {
    _replaceBackground: window._replaceBackground,
    _urlRewrite: window._urlRewrite
  };

  // Merge defaults < globals < URL params
  const opts = {
    _replaceBackground: truthy(urlParams._replaceBackground ?? globalParams._replaceBackground ?? DEFAULTS._replaceBackground),
    _urlRewrite:        truthy(urlParams._urlRewrite        ?? globalParams._urlRewrite        ?? DEFAULTS._urlRewrite)
  };
  ...
  // Observer hides any late-inserted host nodes
  ...
  const init = () => {
    ...
    root.innerHTML = `
      ...
      <div class="xss-root" id="xssStage">
        ...
        <div class="xss-dim" style="${opts._replaceBackground ? '' : 'background:#1a1a1a;'}"></div>
